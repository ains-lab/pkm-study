---
title: "ains-lab/2nd-brain-template: llm-wiki 이용하여 나의 개인 지식관리 생성을 위한 2nd-brain 프로젝트 입니다."
source: "https://github.com/ains-lab/2nd-brain-template"
author:
published:
created: 2026-07-29
description: "llm-wiki 이용하여 나의 개인 지식관리 생성을 위한 2nd-brain 프로젝트 입니다. - ains-lab/2nd-brain-template"
tags:
  - "clippings"
---
## 2nd Brain Template

**English** | [한국어](https://github.com/ains-lab/2nd-brain-template/blob/main/README.ko.md)
...
The [complete architecture](https://github.com/ains-lab/2nd-brain-template/blob/main/docs/architecture/second-brain-pkm-architecture.md) consists of four layers: **Evidence → Canonical Memory → Discovery → Human Decision**. Original content and metadata are preserved as immutable evidence, while only reusable knowledge is compiled into canonical Markdown with traceable sources. Relationships discovered through NotebookLM and the knowledge graph remain hypotheses until human verification promotes them into durable memory.

[![Evidence-based personal knowledge management architecture for the 2nd Brain](https://github.com/ains-lab/2nd-brain-template/raw/main/docs/architecture/second-brain-pkm-architecture.png)](https://github.com/ains-lab/2nd-brain-template/blob/main/docs/architecture/second-brain-pkm-architecture.png)

### Operating Workflow

The [operating workflow](https://github.com/ains-lab/2nd-brain-template/blob/main/docs/architecture/second-brain-pkm-architecture.md#6-%ED%95%B5%EC%8B%AC-%EC%9B%8C%ED%81%AC%ED%94%8C%EB%A1%9C%EC%9A%B0) follows **Capture → Compile → Discovery → Human Decision**. Each stage must pass integrity, frontmatter, and structural validation gates before moving forward. Approved changes update the canonical documents, index, and log together, then feed back into the knowledge graph and reusable outputs.

[![2nd-Brain Evidence to Reusable Knowledge operating workflow](https://github.com/ains-lab/2nd-brain-template/raw/main/docs/workflow/second-brain-workflow.png)](https://github.com/ains-lab/2nd-brain-template/blob/main/docs/workflow/second-brain-workflow.png)

### Technology Stack

The core assets in the [technology stack](https://github.com/ains-lab/2nd-brain-template/blob/main/docs/architecture/second-brain-pkm-architecture.md#5-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D) are not particular products, but **open-format source material, canonical Markdown, provenance metadata, and Git history**. Obsidian, Zotero, NotebookLM, and Understand Anything are replaceable tools for capture, editing, discovery, and analysis. Integrity checks and human approval gates connect the stack.

[![2nd-Brain Durable Knowledge technology stack](https://github.com/ains-lab/2nd-brain-template/raw/main/docs/tech-stack/second-brain-technology-stack.png)](https://github.com/ains-lab/2nd-brain-template/blob/main/docs/tech-stack/second-brain-technology-stack.png)
...
| **Verified knowledge compilation** | [LLM Wiki](https://github.com/ains-lab/2nd-brain-template/blob/main/concepts/llm-wiki.md) structures source material into concept, comparison, and query documents while accumulating provenance, confidence, contradictions, and relationships. |
| **Connected exploration and editing** | Follow the [second-brain research workflow](https://github.com/ains-lab/2nd-brain-template/blob/main/concepts/second-brain-research-workflow.md) to read and edit durable knowledge in Obsidian using Markdown, wikilinks, and backlinks. |
| **Source-grounded focused research** | Use the [NotebookLM query compounding workflow](https://github.com/ains-lab/2nd-brain-template/blob/main/queries/notebooklm-query-compounding.md) to question a constrained source set and file only results whose reuse value has been verified. |
| **Knowledge graph analysis** | Use the [UA knowledge graph workflow](https://github.com/ains-lab/2nd-brain-template/blob/main/queries/ua-knowledge-graph-workflow.md) to find clusters, bridges, isolated documents, and possible knowledge gaps, then verify graph results against the source material. |
| **Human verification and feedback** | The [research feedback loop](https://github.com/ains-lab/2nd-brain-template/blob/main/concepts/research-feedback-loop.md) classifies discoveries as accepted, contested, deferred, or rejected and feeds only approved knowledge back into the index and change history. |
...
> \[!IMPORTANT\] Install for your agent environment MCP configuration files, project and local skill paths, plugin support, and restart procedures differ between agents. Read the official installation documentation linked below and choose the method supported by your current agent or MCP client. Do not copy commands or configuration intended for a different agent without adapting them.
...
The repository root is both the wiki root and the Obsidian vault. All paths are resolved relative to this root without a separate database, and [SCHEMA.md](https://github.com/ains-lab/2nd-brain-template/blob/main/SCHEMA.md) defines the validity contract for directories and data.
...
Before adding knowledge, read [SCHEMA.md](https://github.com/ains-lab/2nd-brain-template/blob/main/SCHEMA.md), check [index.md](https://github.com/ains-lab/2nd-brain-template/blob/main/index.md) for subjects already covered, and review the latest entries in [log.md](https://github.com/ains-lab/2nd-brain-template/blob/main/log.md). If the subject already exists, strengthen the existing page with new evidence instead of creating a similar duplicate.
...
> [!important] Important
> `README.md` is a usage guide; [SCHEMA.md](https://github.com/ains-lab/2nd-brain-template/blob/main/SCHEMA.md) is the authoritative data contract. Follow `SCHEMA.md` whenever the two appear to differ.
...
Before adding a new source type, tag, or automation, register its role and integrity rules in [SCHEMA.md](https://github.com/ains-lab/2nd-brain-template/blob/main/SCHEMA.md). When adding a source directory, define the file formats that qualify as canonical `sources` and the path-preservation policy. Register new tags in the taxonomy before using them.
